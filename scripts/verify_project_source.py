...
p1report = (root / "app/src/main/java/com/example/virtualandroid/vm/P1RunReport.kt").read_text()
p1store = (root / "app/src/main/java/com/example/virtualandroid/vm/P1ReportStore.kt").read_text()
p1contract = (root / "app/src/main/java/com/example/virtualandroid/vm/P1ProofContract.kt").read_text()
p1smoke = (root / "scripts/adb_smoke_p1.sh").read_text()
p1validator = (root / "scripts/validate_p1_report.py").read_text()
guestinstaller = (root / "app/src/main/java/com/example/virtualandroid/vm/GuestAssetInstaller.kt").read_text()
...
require('P1ProofContract.TERMINAL_MARKER' in service, "P1 acceptance marker handling missing")
...
require('P1ProofContract.TERMINAL_MARKER' in p1report and 'line.trim() == P1ProofContract.TERMINAL_MARKER' in service, "P1 exact terminal marker contract missing")
require('put("runId", runId)' in p1report and 'backend' in p1report, "P1 report must record runId/backend")
require('qemuSha256' in p1report and 'kernelSha256' in p1report and 'buildFingerprint' in p1report, "P1 proof hashes/device build identity missing")
require('startedAt < packageInfo.lastUpdateTime' in p1store, "P1 store must reject proof predating current APK")
require('--expected-run-id "$RUN_ID"' in p1smoke and 'Could not clear stale latest-p1.json' in p1smoke, "P1 smoke must enforce current-run anti-stale proof")
require('--require-success' in p1smoke and 'terminalMarkerSeen' in p1validator, "P1 smoke must use strict report validator")
require('installRequiredAsset(context, "p1/Image", kernel)' in guestinstaller, "P1 guest assets must be refreshed from current APK")
require('String runId' in aidl, "P1 Binder API must carry acceptance runId")
